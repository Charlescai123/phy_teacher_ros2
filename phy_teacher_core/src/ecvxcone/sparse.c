...
extern void (*axpy_[])(int *, void *, void *, int *, void *, int *) ;
extern void (*gemm_[])(char *, char *, int *, int *, int *, void *,
...
extern void (*scal_[])(int *, number *, void *, int *) ;
extern void (*gemm_[])(char *, char *, int *, int *, int *, void *, void *,
    int *, void *, int *, void *, void *, int *) ;
extern void (*syrk_[])(char *, char *, int *, int *, void *, void *,
    int *, void *, void *, int *) ;
...
//   if (id != MAX(id,src->id)) ERR_TYPE("incompatible matrix types");
...
spmatrix *SpMatrix_NewFromMatrix(matrix *src, int id)
{
  ...
  if (id < MAT_ID(src)) ERR_TYPE("illegal type conversion");
...
//       } else {
//         if (blk_ncols != nk)
//           ERR_TYPE("incompatible dimensions of subblocks");
//         mk += blk_nrows;
//       }
//     }
//     if (j==0)
//       m = mk;
//     else if (m != mk) ERR_TYPE("incompatible dimensions of subblocks");
//   }

//   if ((id_arg >= 0) && (id_arg < id))
//     ERR_TYPE("illegal type conversion");
...
ccs * transpose(ccs *A, int conjugate) 
{
...
//     int mn_int = (int) mn;
//     scal_[Y->id](&mn_int, &a, Z->values, &intOne);
...
static int inline sp_dgemv(char tA, int m, int n, number alpha, void *a, int oA,
    void *x, int ix, number beta, void *y, int iy)
{
  ccs *A = a;
  double *X = x, *Y = y;

  scal_[A->id]((tA == 'N' ? &m : &n), &beta, Y, &iy);
...
static int inline sp_zgemv(char tA, int m, int n, number alpha, void *a, int oA,
    void *x, int ix, number beta, void *y, int iy)
{
  ccs *A = a;
#ifndef _MSC_VER
    ...
    void **z, int m, int n, int k)
{
  ...
  if (sp_a && sp_b && sp_c && partial) {
    ...
    *z = Z;
    ...
    int j, l, mn_ = m*n;
    scal_[DOUBLE](&mn_, &beta, C, &intOne);

    for (j=0; j<n; j++) {
      for (l=B->colptr[j]; l<B->colptr[j+1]; l++) {
        double a_ = alpha.d*((double *)B->values)[l];
        axpy_[DOUBLE](&m, &a_, A + (tA=='N' ? B->rowind[l]*m : B->rowind[l]),
    ...
    for (j=0; j<A->ncols; j++) {
      for (l=A->colptr[j]; l<A->colptr[j+1]; l++) {
        ...
        axpy_[DOUBLE](&n, &a_, B + (tB == 'N' ? j : j*n), &ib,
            C + A->rowind[l], &m);
      }
    }
    if (A != a) free_ccs(A);
    ...
    *z = Z;
    ...
    gemm_[DOUBLE](&tA, &tB, &m, &n, &k, &alpha, A, &ldA,
    ...
    int m, int n, int k)
{
  ...
  if (sp_a && sp_b && sp_c && partial) {
    ...
    *z = Z;
    ...
    int i, j, l, mn_ = m*n;
    scal_[COMPLEX](&mn_, &beta, C, &intOne);
    ...
    *z = Z;
    ...
    gemm_[COMPLEX](&tA, &tB, &m, &n, &k, &alpha, A, &ldA,
  ...
  if (sp_a && sp_c && partial) {
    ...
    *z = Z;
    ...
    for (j=0; j<B->ncols; j++) {
      ...
      if (uplo == 'U') {
        int m = j+1;
        scal_[DOUBLE](&m, &beta, C + j*n, &intOne);
        ...
        int m = n-j;
        scal_[DOUBLE](&m, &beta, C + j*(n+1), &intOne);
    ...
    syrk_[DOUBLE](&uplo, &trans, &n, &k, &alpha, A,
...
{
  spmatrix *ret;
  if (!(ret = (spmatrix *)malloc(sizeof(spmatrix))))
    err_no_memory;

  ret->mat_type = MAT_SPARSE;
  ret->obj = alloc_ccs(nrows, ncols, nnz, id);
  if (!ret->obj) err_no_memory;

  return ret;
}

spmatrix * SpMatrix_NewFromCCS(ccs *x)
{
  spmatrix *ret = (spmatrix *)malloc(sizeof(spmatrix));
...
spmatrix * SpMatrix_NewFromSpMatrix(spmatrix *A, int id)
{
  if ((id == DOUBLE) && (SP_ID(A) == COMPLEX))
    ERR_TYPE("cannot convert complex to double");
...
//     ERR_TYPE("index sets I and J must be integer");

//   if (MAT_LGT(Il) != MAT_LGT(Jl))
//     ERR_TYPE("index sets I and J must be of same length");

//   if (V && !Matrix_Check(V)) ERR_TYPE("invalid V argument");

//   if (V && Matrix_Check(V) && (MAX(id,MAT_ID(V)) != id))
//     ERR_TYPE("matrix V has invalid type");

//   if (V && (MAT_LGT(V) != MAT_LGT(Il)))
//     ERR_TYPE("I, J and V must have same length");
...
//   if (m < Imax+1 || n < Jmax+1) ERR_TYPE("dimension too small");

//   for (k=0; k<MAT_LGT(Il); k++)
//     if ((MAT_BUFI(Il)[k] < 0) || (MAT_BUFI(Il)[k] >= m) ||
//         (MAT_BUFI(Jl)[k] < 0) || (MAT_BUFI(Jl)[k] >= n) )
//       ERR_TYPE("index out of range");
...
//     ERR_TYPE("V must be either a sequence type, a matrix, or a number");
//   }

//   if (size && !PyArg_ParseTuple(size, "nn", &nrows, &ncols))
//     ERR_TYPE("invalid dimension tuple");

//   if (size && (nrows < 0 || ncols < 0))
//     ERR_TYPE("dimensions must be non-negative");

//   if (tc && !(VALID_TC_SP(tc))) ERR_TYPE("tc must be 'd' or 'z'");
...
//       return NULL;
//   }
//   else ERR_TYPE("invalid type for I");
...
//     Py_DECREF(cvxopt);
//     ERR(PyExc_KeyError, "missing 'spmatrix_str' in 'cvxopt'");
//   }

//   Py_DECREF(cvxopt);
//   if (!PyCallable_Check(str)) ERR_TYPE("'spmatrix_str' is not callable");
...
//     Py_DECREF(cvxopt);
//     ERR(PyExc_KeyError, "missing 'spmatrix_repr' in 'cvxopt'");
//   }

//   Py_DECREF(cvxopt);
//   if (!PyCallable_Check(repr)) ERR_TYPE("'spmatrix_repr' is not callable");
...
// {
//   if (!value) ERR_INT(PyExc_TypeError,"size attribute cannot be deleted");

//   if (!PyTuple_Check(value) || PyTuple_Size(value) != 2)
//     ERR_INT(PyExc_TypeError, "can only assign a 2-tuple to size");
...
// #endif
//     ERR_INT(PyExc_TypeError, "invalid size tuple");
...
//   if (m<0 || n<0)
//     ERR_INT(PyExc_TypeError, "dimensions must be non-negative");

//   if (m*n != SP_NROWS(self)*SP_NCOLS(self))
//     ERR_INT(PyExc_TypeError, "number of elements in matrix cannot change");

//   int_t *colptr = calloc((n+1),sizeof(int_t));
//   if (!colptr) ERR_INT(PyExc_MemoryError, "insufficient memory");
...
// {
//   if (!value) ERR_INT(PyExc_AttributeError, "attribute cannot be deleted");

//   if (PY_NUMBER(value)) {
//     number val;
//     if (convert_num[SP_ID(self)](&val, value, 1, 0))
//       ERR_INT(PyExc_TypeError, "invalid type in assignment");
...
//     return 0;
//   } else ERR_INT(PyExc_TypeError, "invalid assignment for V attribute");
// }
...
spmatrix * spmatrix_trans(spmatrix *A) 
{
  ...
  ccs *T = transpose(A->obj, 0);
  if (!T) return NULL;
  
  spmatrix *B = malloc(sizeof(spmatrix));
  if (!B) {
      free_ccs(T);
      err_no_memory;
  }

  B->mat_type = MAT_SPARSE;
  B->obj = T;
  return B;

}

spmatrix *spmatrix_ctrans(spmatrix *A) {
    if (!A || !A->obj) return NULL;

    ccs *T = transpose(A->obj, 1); 
    if (!T) return NULL;
...
//       ERR(PyExc_IndexError, "index out of range");
...
//     ERR(PyExc_TypeError, "invalid index sets I or J");
...
//       colstep = 1;
//     }
//     else ERR_TYPE("invalid index argument");
...
//   if (!value) ERR_INT(PyExc_NotImplementedError,
...
//     if (!value)
//       ERR_INT(PyExc_NotImplementedError, "invalid type in assignment");
...
//       ERR_INT(PyExc_TypeError, "invalid argument type");
//     itype = 'n';
//   }
...
// #endif
//     if (itype != 'n')
//       ERR_INT(PyExc_IndexError, "incompatible sizes in assignment");
...
// #endif
//     if ( i<-SP_LGT(self) || i >= SP_LGT(self) )
//       ERR_INT(PyExc_IndexError, "index out of range");
...
//         ERR_INT(PyExc_MemoryError, "Cannot reallocate sparse matrix");
//       spmatrix_setitem_i(self, i, &val);
//     }
//     return 0;
//   }
...
//       if (decref_val) { Py_DECREF(value); }
//       ERR_INT(PyExc_TypeError, "incompatible sizes in assignment");
//     }
...
//         if (decref_val) { Py_DECREF(value); }
//         ERR_INT(PyExc_MemoryError, "insufficient memory");
//       }
...
//   PyObject *argI = NULL, *argJ = NULL;
//   if (!PyArg_ParseTuple(args, "OO", &argI, &argJ))
//     ERR_INT(PyExc_TypeError, "invalid index arguments");
...
//     if (itype != 'n')
//       ERR_INT(PyExc_TypeError, "argument has wrong size");
...
//     if (decref_val) { Py_DECREF(value); }
//     ERR_INT(PyExc_TypeError, "incompatible size of assignment");
//   }
...
//   int n=SP_NNZ(x);
//   scal_[SP_ID(self)](&n, &MinusOne[SP_ID(self)], SP_VAL(x), &intOne);
...
// {
//   if (!SpMatrix_Check(other))
//     ERR_TYPE("invalid inplace operation");

//   int id = SP_ID(self);
//   if (SP_ID(other) > id)
//     ERR_TYPE("incompatible types for inplace operation");
...
//       scal_[id](&n, &MinusOne[id], MAT_BUF(ret), &intOne);
//       return ret;
//     }
//     else return NULL;
//   }
...
//     scal_[id]((int *)&SP_NNZ(ret), &val, SP_VAL(ret), (void *)&One[INT]);
//     return (PyObject *)ret;
//     }
...
// {
//   if (!(PY_NUMBER(other) || (Matrix_Check(other) && MAT_LGT(other) == 1)))
//     ERR_TYPE("invalid operands for sparse multiplication");

//   if (SP_ID(self) < get_id(other, PY_NUMBER(other)))
//     ERR_TYPE("invalid operands for inplace sparse multiplication");

//   number val;
//   convert_num[SP_ID(self)](&val, other, !Matrix_Check(other), 0);
//   scal_[SP_ID(self)]((int *)&SP_NNZ(self), &val, SP_VAL(self),
//       (void *)&One[INT]);
...
//     ERR_TYPE("invalid operands for sparse division");
...
//     return ret;
//   } else {
//     if (id != idA) ERR_TYPE("invalid inplace operation");
